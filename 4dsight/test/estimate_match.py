...
class PoseEstimator(object):
    ...
    def add_target(self, image, rect, data=None):
        print("poseestimator-add_target")
        x_start,y_start,x_end,y_end = rect
        ...
        target = self.cur_target(image=image,
                                 rect=rect,
                                 keypoints=keypoints,
    ...
    def track_target(self,frame):
        print("poseestimator-track_target")
        ...
        for image_index,matches in enumerate(matches_using_index):
            ...
            quad = np.float32([[x_start, y_start],
                               [x_end, y_start],
                               [x_end, y_end],
                               [x_start, y_end]])
    ...
    def detect_features(self,frame):
        print("poseestimator-dtecet_features")
    ...
    def clear_targets(self):
        print("poseestimator-clear_targets")
...
class VideoHandler(object):
    def __init__(self,capId,scaling_factor,win_name):
        ...
        self.rect = None
        self.frame = cv2.resize(frame, None,
                                fx=scaling_factor,
                                fy=scaling_factor,
    ...
    def set_rect(self,rect):
        print("videohandler-set_rect")
        self.rect = rect
        self.pose_tracker.add_target(self.frame, rect)

    def start(self):
        print("videohandler-start")
        paused = False
        while True:
            ...
            if not paused and self.rect is not None:
                ...
                for item in tracked:
                    ...
                    for (x, y) in np.int32(item.points_cur):
                        cv2.circle(img, (x, y), 3, (25, 250, 25))
...
if __name__ == '__main__':
    print("press left button and ")
    print("dragging mouse!!!!")
    v=VideoHandler(0,0.95,'Tracker')
    v.start()
